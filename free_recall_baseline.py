...
def run_experiment():
    ...
    for p in range(1):
        ...
        # Prepare CSV file
        with open(f"Result/free_recall_baseline_{name}.csv", "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow([
                "participant",
                "trial",
    ...
    print(f"\nExperiment complete. Data saved to Result/free_recall_baseline_{name}.csv")
